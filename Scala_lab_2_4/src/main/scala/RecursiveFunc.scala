...
object RecursiveFunctions {
  ...
  def Reverse[A](list: List[A]): List[A] =  {
    @tailrec
    def loop(l: List[A], num: List[A]): List[A] = l match {
      case Nil() => num
   ...
   */
  def MapF[A, B](list: List[A], f: A => B): List[B] =  {
    @tailrec
  ...
  def testMap[A, B](list: List[A], f: A => B): List[B] =  MapF(list, f)
  ...
  def FlatMapF[A, B](list: List[A])(f: A=>List[B]): List[B] = {
    @tailrec
  ...
  def testFlatMap[A, B](list: List[A], f: A => List[B]): List[B] = FlatMapF(list)(f)
