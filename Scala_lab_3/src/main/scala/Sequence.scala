...
//  val revSeq[A] = (seq: Seq[A]) => for(i <- seq.length - 1 to 0) yield seq(i)
  def testPalindrom[A](seq: Seq[A]): Boolean = seq.reverse == seq

  /* e) Реализуйте flatMap используя foldLeft.
   *    
   */
  def testFlatMap[A, B](seq: Seq[A])(f: A => Seq[B]): Seq[B] = seq.map(f).foldLeft(Seq[B]())((outCurr, outAcc) => outCurr ++ outAcc)
}
